% devti_block_header.m
%
% this script generates the header file 'header' in the  workspace
% the 'header' variable is used when calling DevTI_block experiment
%
% this experiment is a blocked version of the AI task used in DevTI
% (behavioral only). the triads are the same, with just the order of
% study/test being different across the two tasks.
%
...
%
    ...
    % devti_block_ai_part2.m
% must be called from DevTI_block experiment's folder to work.
%
% MLS version 1, July 2015
%
% dependencies:
%   PsychToolbox
clear;
...
addpath(path,'-begin')
%% set-up
header = struct('exp', 'DevTI_block','version','MLS v.1 | July 2015','parameters',struct);
header.path.exp = pwd;
disp('This script generates a header file for DevTI_block experiment.');
disp('--------------------------------------------------------------');
...
par.ai.study.nStudy = 8; % 4 reps each of blocked and intermixed
par.ai.study.nRepsPerPair = par.ai.study.nStudy/2 * par.ai.study.nRepsPerPairPerStudy;
...
par.ai.test.nTests = 8; % number of interleaved DIRECT tests (there will be an additional AC test @ end)
par.ai.test.nTrialsPerTest = par.ai.nABCTriads * 1;
par.ai.rightKey = 'k';
par.ai.midKey = 'g';
par.ai.leftKey = 's';
