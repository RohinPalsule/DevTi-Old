function data = devti_block_ai(header)
...
outfname = sprintf('%sai_%03d_%s',header.path.data, header.subNr, header.subInit);
...
for r = 1:par.ai.study.nStudy
    ...
    runNr = r;
    %% IS THIS RIGHT?
    data.study.actualOnsets{runNr} = NaN(par.ai.nABCTriads,1); 
    ...
    data.test.actualOnsets{runNr} = nan(par.ai.nABCTriads,1);
    data.test.resp{runNr} = nan(par.ai.nABCTriads,1);
    data.test.stimRT{runNr} = nan(par.ai.nABCTriads,1);
    data.test.isCorrect{runNr} = nan(par.ai.nABCTriads,1);
